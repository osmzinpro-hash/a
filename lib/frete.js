...
export async function geocodeStreet({ street, number, city, uf }) {
  if (!street) return null;
  const tries = number ? [`${number} ${street}`, street] : [street];
  for (const st of tries) {
    const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&limit=3&countrycodes=br&addressdetails=1`
      + `&street=${encodeURIComponent(st)}&city=${encodeURIComponent(city)}&state=${encodeURIComponent(UF_NAMES[uf] || uf)}`;
    const r = await getJson(url);
    if (!Array.isArray(r)) continue;
    const hit = r.find(x => (x.category === 'highway' || TRUSTED.has(x.addresstype))
      && norm(x.address?.city || x.address?.town || x.address?.municipality || x.address?.village || '') === norm(city));
    if (hit && num(hit.lat)) return { lat: num(hit.lat), lng: num(hit.lon), precision: st === street ? 'rua' : 'numero' };
  }
  return null;
}
...
export async function quoteDelivery({ cep, number, settings, neighborhoods, cache }) {
  ...
  let point = numberKey ? await geo(`geo2:${cep}:${numberKey}`, { ...addr, number: numberKey }) : null;
  if (!point) point = await geo(`geo2:${cep}:rua`, addr);
