...
import { quoteDelivery, deliveryConfig, formatRanges, parseRanges } from './frete.js';

export const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || 'ditos2002';
// Alternativa à senha em texto: o SHA-256 dela (útil em prévias, onde a configuração pode ficar visível).
const ADMIN_HASH = String(process.env.ADMIN_PASSWORD_SHA256 || '').trim().toLowerCase();
const SECRET = process.env.SESSION_SECRET || createHmac('sha256', 'ditos-lanches').update(ADMIN_HASH || ADMIN_PASSWORD).digest('hex');
const PREVIEW = process.env.DITOS_PREVIEW === '1';
const TZ = 'America/Sao_Paulo';

if (!process.env.ADMIN_PASSWORD && !ADMIN_HASH) {
...
const passwordOk = input => ADMIN_HASH
  ? sameText(createHash('sha256').update(String(input || '')).digest('hex'), ADMIN_HASH)
  : sameText(input || '', ADMIN_PASSWORD);
...
const EDITABLE_SETTINGS = [...PUBLIC_SETTINGS, 'pix_enabled', 'pix_key', 'pix_name', 'pix_city', 'store_mode', 'open_time', 'close_time', 'open_days', 'closed_message', 'delivery_ranges', 'store_lat', 'store_lng'];
const PAYMENTS = {
  pix: 'Pix pelo site',
...
export function createApi({ onCatalogChange = () => {} } = {}) {
  ...
  // Cache no banco para as consultas de CEP e mapa (30 dias).
  const cacheFor = db => ({
    get: async key => {
      const row = await db.get('SELECT data, at FROM lookup_cache WHERE key = ?', [key]);
      return row && Date.now() - Date.parse(row.at) < 30 * 86400e3 ? JSON.parse(row.data) : null;
    },
    set: async (key, value) => {
      await db.run('INSERT INTO lookup_cache (key, data, at) VALUES (?, ?, ?) ON CONFLICT (key) DO UPDATE SET data = excluded.data, at = excluded.at', [key, JSON.stringify(value), new Date().toISOString()]);
    }
  });
  async function quote(db, cep, number, s) {
    const neighborhoods = await db.all('SELECT name, fee_cents, eta, active FROM neighborhoods');
    return quoteDelivery({ cep, number, settings: s || await getSettings(db), neighborhoods, cache: cacheFor(db) });
  }
  ...
  async function menuPayload(db) {
    ...
    return {
      store: {
        ...
        delivery_max_km: deliveryConfig(s).ranges.at(-1).km,
        preview: PREVIEW
      },
      categories,
  ...
  async function createOrder(db, input) {
    ...
    if (fulfillment === 'delivery') {
      // O frete sai do CEP, recalculado aqui: o valor mandado pelo navegador é ignorado.
      const number = clean(input.address?.number, 20);
      const q = await quote(db, input.address?.cep, number, s);
      if (!q.ok) throw httpError(400, q.error);
      address = {
        cep: q.cep.replace(/(\d{5})(\d{3})/, '$1-$2'),
        street: clean(input.address?.street, 120) || q.street, number,
        complement: clean(input.address?.complement, 80), reference: clean(input.address?.reference, 120),
        district: q.district, city: q.city, uf: q.uf, distance_km: q.distance_km ?? null, fee_method: q.method
      };
      if (address.street.length < 3 || !address.number) throw httpError(400, 'Informe rua e número para a entrega.');
      neighborhood = q.district || q.city;
      fee = q.fee_cents;
    }
  ...
  async function handleAdmin(db, req, res, path, url) {
    if (path === '/api/admin/login' && req.method === 'POST') {
      ...
      if (!passwordOk(password)) return fail(res, 401, 'Senha incorreta.');
    ...
    if (path === '/api/admin/catalog' && req.method === 'GET') {
      ...
      const settings = Object.fromEntries(Object.entries(s).filter(([k]) => EDITABLE_SETTINGS.includes(k)));
      const cfg = deliveryConfig(s);
      settings.delivery_ranges = s.delivery_ranges || formatRanges(cfg.ranges);
      settings.store_lat = s.store_lat || String(cfg.store.lat);
      settings.store_lng = s.store_lng || String(cfg.store.lng);
      return send(res, 200, { categories, products, groups, options, neighborhoods, settings });
    }
    ...
    if (path === '/api/admin/settings' && req.method === 'PUT') {
      const b = await readJson(req);
      if (b.delivery_ranges !== undefined && String(b.delivery_ranges).trim() && !parseRanges(b.delivery_ranges).length) {
        return fail(res, 400, 'Faixas de frete inválidas. Use uma por linha: até km | taxa | tempo (ex.: 1 | 7,00 | 20 a 30 min).');
      }
      for (const [k, v] of Object.entries(b)) {
        if (EDITABLE_SETTINGS.includes(k)) await db.run('INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT (key) DO UPDATE SET value = excluded.value', [k, k === 'delivery_ranges' ? String(v ?? '').slice(0, 2000) : clean(v, 300)]);
      }
      onCatalogChange();
  ...
  async function handle(req, res) {
    ...
    try {
      ...
      if (path === '/api/frete' && req.method === 'GET') {
        if (limited('frete:' + clientIp(req), 60, 10 * 60e3)) return fail(res, 429, 'Muitas consultas seguidas. Espere um pouco.');
        const q = await quote(db, url.searchParams.get('cep'), url.searchParams.get('numero'));
        return send(res, q.ok ? 200 : 422, q);
      }
